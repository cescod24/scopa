...
blank.style.height = "25px"


//ai

//qui scegli il livello dei due bot

let botZeroLevel = 4; //tu
let botOneLevel = 1; //avversario
...
function shuffleDeck(deck) { //mischia il mazzo casualmente esegue 5 scambi
...
function changeTurn() { //funzione per cambiare il turno. Chiama anche la funzione che fa giocare il bot
	...
	if(playing && turn == 1) {
		...
		botLevel = botOneLevel
		setTimeout(() => {evaluatePosition()}, 5);

	} else if(playing && turn == 0) {

		botLevel = botZeroLevel
		setTimeout(() => {evaluatePosition()}, 5)
	}
...
function calculatePoints() {	//a fine partita, calcola i punti per ogni player e li mostra sullo schermo
	...
	if(players[0].punti >= 11 && players[0].punti > players[1].punti) {
		...
		setTimeout(() => {newRoundButton.click()}, 150)
...
function HTMLhandler() { //gestisce i bottoni
	...
	document.getElementById("yourButton").addEventListener("click", function() {
		...
		if (clickedCardsTable != 0) {
			...
			setTimeout(() => {takeCards(clickedCardsHand, clickedCardsTable)}, 5)

		} else {

			setTimeout(() => {placeCard(clickedCardsHand)}, 5)
...
function createWindow() { //crea la pagina per il gioco
	...
	rightContainer.style = "display: flex; justify-content: flex-end; width: 15vw; height: 5vh; flex-direction: column;"
	centerContainer.style = "display: flex; height: 5vh; width: 80vw; flex-direction: column; align-items: center; justify-content: space-around; flex-wrap: nowrap;"
	leftContainer.style = "display: flex; justify-content: center; width: 15vw; height: 5vh; flex-direction: column;"

	messaggiDestriSopraSopra.style = "font: 5px Georgia, serif; color: black; height: 30vh; text-align: center;"
	messaggiDestriSopra.style = "font: 5px Georgia, serif; color: orange; height: 30vh; text-align: center;"
	messaggiDestriSotto.style = "font: 34px Georgia, serif; color: blue; height: 25vh; text-align: center;"
	messaggiSinistriSopra.style = "font: 5px Georgia, serif; color: red; height: 10vh; text-align: center;"
	messaggiSinistriSotto.style = "font: 5px Georgia, serif; color: red; height: 10vh; text-align: center;"
...
function evaluatePosition() { //questa funzione calcola le migliori carte da giocare, secondo certi criteri
	...
	if (!takable) {
		...
		for(pseudoHandCard of players[turn].hand) {
			...
			if(botLevel > 3) { //il bot di livello 4 conosce la mano dell'avversario!!!
				...
				players[(turn + 1) % 2].hand.forEach((element) => {

					if(Object.values(element).includes(pseudoHandCard.value)) {
					...
					if(checkTakeability(element, WithCombination) == 0) {
		...
		setTimeout(() => {placeCard(bestCard)}, 5);
				...
				if(checkTakeability(pseudoHandCard, combination) == 0) {
					...
					if(botLevel > 3) { //feature solo per il bot di livello 4: se dopo aver preso, puoi far scopa, la valutazione si abbassa di molto 
						...
						players[(turn + 1) % 2].hand.forEach((card) => {
							...
							if(checkTakeability(card, WithoutCombination) == 0) {
	...
	setTimeout(() => {takeCards(bestCard, bestTableCards)}, 5);
